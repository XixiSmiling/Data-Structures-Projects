...
public class RunIterator implements Iterator {
   ...
   */
	private DList<Run> list;
	private DListNode<Run> cur;
  ...
  public boolean hasNext() {
    if(list != null && !list.isEmpty() && cur != list.back()) {
    	return true;
    }
    return false;
  }
  ...
  public int[] next() {
	...
	if(hasNext()) {
		...
		return new int[]{cur.item.length, cur.item.red, cur.item.green, cur.item.blue};
	}
    return null;
  }
