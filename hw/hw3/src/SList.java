...
public class SList {
  ...
  public void squish() {
    // Fill in your solution here.  (Ours is eleven lines long.)
	  if(this.size <= 1) { // special case;
		  return;
	  }
	  SListNode currentNode = this.head;
	  this.size = 1;
	  for(SListNode testNode = this.head; testNode != null; testNode = testNode.next) {
		  if(!testNode.item.equals(currentNode.item)) {
...
//	  size = 0;
	  for(SListNode currentNode = head; currentNode != null; currentNode = currentNode. next) {
