...
public class Set {
  ...
  public void insert(Comparable c) {
    // Your solution here.
	try{
		for(ListNode cur = list.front(); cur.item() != null; cur = cur.next()) {
  ...
  public void union(Set s) {
	...
	try {
		for(ListNode thisNode = list.front(); thisNode.isValidNode(); thisNode = thisNode.next()) {
  ...
  public void intersect(Set s) {
    // Your solution here.
	  ...
	  try {
			for(ListNode sNode = s.elements().front(); sNode.isValidNode(); sNode = sNode.next()) {
				while(((Comparable)thisNode.item()).compareTo(sNode.item()) < 0) {
					thisNode = thisNode.next();
					thisNode.prev().remove();
				} 
				if(((Comparable)thisNode.item()).compareTo(sNode.item()) == 0) {
					thisNode = thisNode.next();
				}
			}
  ...
  public String toString() {
    // Replace the following line with your solution.
	StringBuilder result = new StringBuilder(3 + 2 * this.cardinality());
	result.append('{');
	try{
		for(ListNode cur = list.front(); cur.item() != null; cur = cur.next()) {
			result.append(' ').append(cur.item());
		}		
	} catch(InvalidNodeException e) {
		result.append(" }");
	}
  ...
  public static void main(String[] argv) {
    ...
    System.out.println("After s.union(s2), s2 = " + s2);

    s.intersect(s3);
    System.out.println("After s.intersect(s3), s = " + s);

    System.out.println("s.cardinality() = " + s.cardinality());
    System.out.println("After s.intersect(s3), s3 = " + s3);
