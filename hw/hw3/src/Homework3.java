...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
	  if(ints == null || ints.length == 1) {    // special case
		  return;
	  }
	  int smooshedIndex = 0;
	  for(int testIndex = 1; testIndex < ints.length; testIndex++) {  // smoosh
		  if(ints[testIndex] != ints[testIndex - 1]) {
			  ints[++smooshedIndex] = ints[testIndex];
		  }
	  }
	  for(smooshedIndex += 1; smooshedIndex < ints.length; smooshedIndex++) { // fill unused element with -1
		  ints[smooshedIndex] = -1;
	  }
  }
  ...
  public static void main(String[] args) {
    ...
    System.out.println("squishing " + list5.toString() + " (size: " + list5.length() + ")" + ":");
    list5.squish();
    result = list5.toString();
    System.out.println(result + " (size: " + list5.length() + ")");
    ...
    System.out.println("squishing " + list6.toString() + " (size: " + list6.length() + ")" + ":");
    list6.squish();
    result = list6.toString();
    System.out.println(result + " (size: " + list6.length() + ")");
    ...
    System.out.println("squishing " + list7.toString() + " (size: " + list7.length() + ")" + ":");
    list7.squish();
    result = list7.toString();
    System.out.println(result + " (size: " + list7.length() + ")");
    ...
    System.out.println("squishing " + list8.toString() + " (size: " + list8.length() + ")" + ":");
    list8.squish();
    result = list8.toString();
    System.out.println(result + " (size: " + list8.length() + ")");
    TestHelper.verify(result.equals("[  0  1  2  3  4  5  6  ]"),
                      "BAD SQUISH!!!  No biscuit.");

    SList list9 = new SList();
    System.out.println("squishing " + list9.toString() + " (size: " + list9.length() + ")" + ":");
    list9.squish();
    result = list9.toString();
    System.out.println(result + " (size: " + list9.length() + ")");
    TestHelper.verify(result.equals("[  ]"),
                      "BAD SQUISH!!!  No biscuit.");
    ...
    System.out.println("twinning " + list6.toString() + " (size: " + list6.length() + ")" + ":");
    list6.twin();
    result = list6.toString();
    System.out.println(result + " (size: " + list6.length() + ")");
    TestHelper.verify(result.equals(
    ...
    System.out.println("twinning " + list7.toString() + " (size: " + list7.length() + ")" + ":");
    list7.twin();
    result = list7.toString();
    System.out.println(result + " (size: " + list7.length() + ")");
    ...
    System.out.println("twinning " + list9.toString() + " (size: " + list9.length() + ")" + ":");
    list9.twin();
    result = list9.toString();
    System.out.println(result + " (size: " + list9.length() + ")");
    TestHelper.verify(result.equals("[  ]"),
